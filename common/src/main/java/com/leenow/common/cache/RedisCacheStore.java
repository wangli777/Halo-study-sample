...
public class RedisCacheStore extends AbstractStringCacheStore {
    ...
    private void init() {
        ...
        JEDIS = new JedisPool(cfg, "127.0.0.1", 6379, 5000);
...
//        log.info("Initialized cache redis cluster: {}", JEDIS.getClusterNodes());
    ...
    Optional<CacheWrapper<String>> getInternal(String key) {
        ...
        String value = JEDIS.getResource().get(key);
    ...
    void putInternal(String key, CacheWrapper<String> cacheWrapper) {
        ...
        try {
            ...
            JEDIS.getResource().set(key, JsonUtils.objectToJson(cacheWrapper));
            //设置过期时间
            Date expireAt = cacheWrapper.getExpireAt();
            if (expireAt != null) {
                JEDIS.getResource().pexpireAt(key, expireAt.getTime());
            }
    ...
    Boolean putInternalIfAbsent(String key, CacheWrapper<String> cacheWrapper) {
        ...
        try {
            //存储entry
            if (JEDIS.getResource().setnx(key, JsonUtils.objectToJson(cacheWrapper)) <= 0) {
                //存储失败
